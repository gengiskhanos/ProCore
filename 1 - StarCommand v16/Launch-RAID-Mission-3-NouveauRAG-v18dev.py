...
def valider_qualite_chapitre(titre, reponse):
    ...
    SEUILS_MIN_MOTS = {
        "Chapitre 1 : Identité du Projet": 150,
        "Chapitre 2 : Architecture générale": 180,
        "Chapitre 3A : Workflow détaillé": 150,
        "Chapitre 3B : Vigilance, Interfaces & Spécificités": 120,
        "Chapitre 4 : Cartographie des Données (Technique)": 150,
        "Chapitre 5 : Recommandations": 200,
        "Chapitre 6 : Maintenabilité": 250,
    }
    ...
    # v7.1 : Vérification des sections manquantes pour le chapitre 3B
    if titre == "Chapitre 3B : Vigilance, Interfaces & Spécificités":
        ...
        if sections_trouvees == 0:
            issues.append(
                "Chapitre 3B incomplet : il manque les sections Points de vigilance / Interfaces Inter-outils"
            )
...
def interroger_agent_sse(
    ...
    body = {
        ...
        "userId": RUN_ID,
        "sse": True,
        "projectConfigOverride": {"ai": {"max_tokens": 6000, "history": False}},
    }
...
REGLE_ANTI_BOUCLE = (
    "\n<regle_formatage>\n"
    "RÈGLES MARKDOWN POUR LES TABLEAUX :\n"
    ...
    "RÈGLE POUR LE TEXTE :\n"
    "- Privilégie la prose développée et argumentée plutôt que des listes sèches.\n"
    "- Chaque section doit apporter suffisamment de contexte pour être comprise "
    "par un mainteneur qui découvre le projet.\n"
    "</regle_formatage>\n"
)
...
def construire_prompt_chapitre(
    ...
    CHAPITRES_RAG_INTENSIF = {
        "Chapitre 3A : Workflow détaillé",
        "Chapitre 3B : Vigilance, Interfaces & Spécificités",
    ...
    structures = {
        "Chapitre 1 : Identité du Projet": (
            ...
            "- **Mission** : Résumé détaillé du besoin métier en 5-8 phrases. "
            "Explique POURQUOI ce projet existe, quel problème métier il résout, "
            "pour qui il a été conçu, et comment il s'inscrit dans l'organisation. "
            "Développe le contexte opérationnel.\n"
            "- **Périmètre** : Développe en 3-5 phrases la fréquence d'usage, "
            "la population cible (nombre estimé d'utilisateurs, profils), "
            "le type de données traitées (nature, sensibilité RGPD, volume estimé), "
            "et le périmètre géographique ou organisationnel.\n"
            "- **Historique** : Date de mise en service, statut actuel, "
            "évolutions connues depuis la première version.\n"
            "</structure>\n"
            "Développe chaque section avec suffisamment de contexte pour qu'un nouveau "
            "mainteneur comprenne le projet sans autre document. "
        ...
        "Chapitre 2 : Architecture générale": (
            ...
            "<structure>\n"
            "- **Technologies** : Pour CHAQUE technologie utilisée dans le projet, "
            "rédige un paragraphe de 2-3 phrases décrivant : le nom de la technologie, "
            "son rôle précis dans le projet, et comment elle interagit avec les autres composants. "
            "Présente en liste à puces (pas de tableau).\n"
            "- **Connecteurs et services externes** : Liste les connecteurs identifiés "
            "(SharePoint, Office 365, etc.) avec leur usage précis.\n"
            "- **Schéma de flux (Mermaid)** : Diagramme graph LR avec subgraph. "
            "Chaque arrête (edge) doit être numérotée dans l'ordre "
            'du flux dans son libellé, exemple : A -- "1. Dépose fichier" --> G. '
            "Nœuds avec guillemets doubles. 8-12 étapes numérotées. "
            "Le diagramme doit refléter le flux de données de bout en bout.\n"
            "</structure>\n"
            "Commence directement par le contenu. Pas de titre de chapitre."
        ),
        "Chapitre 3A : Workflow détaillé": (
            "<tache>Génère la SECTION 1 du Chapitre 3 : le Workflow détaillé.\n"
            "INTERDIT : Ne déduis pas les étapes depuis le schéma Mermaid du chapitre 2. "
            "Tu DOIS interroger le RAG pour lire les fichiers sources "
            "avant de rédiger. Le workflow doit comporter 15 à 25 étapes "
            "granulaires reflétant la logique complète du projet (boucles, conditions, appels "
            "de macros, gestion des fichiers).</tache>\n"
            "<structure>\n"
            "**Workflow** : Tableau avec colonnes : Étape | Outil | Action réalisée | Ce qui est impacté.\n"
            "Prend tout le RAG en considération pour rédiger un workflow de maximum 25 lignes. "
            "Cellules courtes et précises. Chaque étape doit être numérotée (1, 2, 3...).\n"
            "Après le tableau, ajoute un paragraphe de synthèse de 2-3 phrases résumant "
            "le flux global du processus.\n"
            "</structure>\n"
            "Commence directement par le tableau. Pas de titre de chapitre."
        ),
        "Chapitre 3B : Vigilance, Interfaces & Spécificités": (
            "<tache>Génère les SECTIONS complémentaires du Chapitre 3 : "
            "Points de vigilance, Interfaces inter-outils, et Spécificités techniques.\n"
            "Tu DOIS interroger le RAG pour extraire les informations factuelles "
            "depuis les fichiers sources (VBA, XAML, formules, connexions).</tache>\n"
            "<structure>\n"
            "SECTION 1 — **Points de vigilance** : Boucles ForEach, conditions If/Case, "
            "fragilités techniques, dépendances fragiles. "
            "Développe chaque point en 1-2 phrases de texte (pas de tableau). "
            "Identifie au minimum 4 points.\n"
            "\n"
            "SECTION 2 — **Interfaces Inter-outils** : Pour chaque transfert de données "
            "entre outils, indique le mécanisme (fichier sur réseau, ouverture Excel, "
            "copier-coller, API...) et si c'est Synchrone ou Asynchrone. "
            "Rédige en texte ou en liste à puces.\n"
            "\n"
            "SECTION 3 — **Spécificités Macro Excel** (si VBA détecté dans les fichiers .xlsm) : "
            ...
            "\n"
            "SECTION 4 — **Spécificités UiPath** (si Main.xaml détecté) : Séquences principales, "
            ...
            "Si VBA ou UiPath ne sont pas détectés, indique-le en une phrase et passe à la suite."
        ),
        "Chapitre 4 : Cartographie des Données (Technique)": (
        ...
        "Chapitre 5 : Recommandations": (
            "<tache>Génère le contenu du Chapitre 5 : Recommandations.\n"
            "Tu DOIS interroger le RAG pour fonder tes recommandations sur "
            "les faiblesses réelles du projet (pas de conseils génériques).</tache>\n"
            "<structure>\n"
            "Produis un tableau avec exactement ces colonnes :\n"
            "| Domaine | Recommandation | Détail technique | Impact attendu | Priorité |\n"
            "| --- | --- | --- | --- | --- |\n"
            "\n"
            "Produis entre 8 et 10 lignes couvrant obligatoirement :\n"
            "- Performance (2 lignes) : cite les contrôles, formules ou requêtes concernés.\n"
            "- Robustesse (2 lignes) : gestion d'erreurs, validations manquantes.\n"
            "- Maintenabilité (2 lignes) : nommage, documentation, structure du code.\n"
            "- Sécurité/RGPD (1-2 lignes) : données personnelles identifiées, accès, rétention.\n"
            "- Évolutivité (1-2 lignes) : limites SharePoint, migration, scalabilité.\n"
            "\n"
            "IMPORTANT : La colonne 'Détail technique' doit citer des éléments concrets "
            "du projet (noms de variables, contrôles, formules, connecteurs) extraits du RAG. "
            "Pas de recommandations vagues.\n"
            "\n"
            "Après le tableau, ajoute un paragraphe de synthèse de 2-3 phrases "
            "hiérarchisant les actions prioritaires.\n"
            "</structure>\n"
            "Commence directement par le tableau. Pas de titre de chapitre."
        ),
        "Chapitre 6 : Maintenabilité": (
            ...
            "<structure>\n"
            "- **Score de Complexité** : [Bas/Moyen/Haut] avec une justification "
            "détaillée de 3-4 phrases expliquant les facteurs de complexité.\n"
            "- **Audit nécessaire ?** : [Oui/Non] avec justification de 2-3 phrases "
            "précisant les points à auditer.\n"
            ...
            "La colonne Justification doit contenir 1-2 phrases argumentées "
            "citant des éléments concrets du projet.\n"
            "- **Points forts** : 4-5 points qui facilitent la reprise. "
            "Développe chaque point en 1-2 phrases avec des exemples concrets.\n"
            "- **Points de fragilité** : 4-5 points de risque technique. "
            "Développe chaque point en 1-2 phrases en citant les éléments concernés.\n"
            "</structure>\n"
            "Commence directement par le contenu. Pas de titre de chapitre."
        ),
    }
...
# Mapping : sous-chapitres → titre Word affiché (regroupement sous un même heading)
CHAPITRES_WORD_HEADING = {
    "Chapitre 3A : Workflow détaillé": "Chapitre 3 : Description étape par étape",
    "Chapitre 3B : Vigilance, Interfaces & Spécificités": None,  # pas de nouveau heading, on continue
}
...
for i, titre_chapitre in enumerate(CHAPITRES_TITRES, start=1):
    ...
    # v17 : Gestion du regroupement Word pour les sous-chapitres 3A/3B
    if titre_chapitre in CHAPITRES_WORD_HEADING:
        heading_word = CHAPITRES_WORD_HEADING[titre_chapitre]
        if heading_word is not None:
            # 3A : on crée le heading principal "Chapitre 3 : Description étape par étape"
            doc_word.add_heading(heading_word, level=1)
        # 3B (heading_word == None) : pas de nouveau heading, on enchaîne directement
    else:
        # Chapitres normaux (1, 2, 4, 5, 6)
        doc_word.add_heading(titre_chapitre, level=1)

    ajouter_texte_markdown(doc_word, reponse)

    # Page break seulement si ce n'est pas 3A (on veut 3A+3B sur la même section)
    if titre_chapitre != "Chapitre 3A : Workflow détaillé":
