...
def enrichir_referentiel(fichier_sortie, dossier_injection='IN\INJECTION-DES-EDI'):
    ...
    # Lire le fichier de sortie existant
...
def comparer_avec_export(fichier_out='OUT\GENERIX', fichier_export='IN/EXPORT-FILIALE-A-COMPARER'):
    ...
    total_lignes_filiale = len(df_export)
    ...
    print(f"\nNombre de TVA valides: {tva_values_clean.notna().sum()} sur {len(tva_values_clean)}")
    print(f"Nombre de SIREN valides: {siren_values_clean.notna().sum()} sur {len(siren_values_clean)}")
    ...
    for idx, (tva, siren) in enumerate(zip(tva_values_clean, siren_values_clean)):
        ...
        if masque.any():
            ...
            found_count += masque.sum()
            if idx < 5:
                print(f"\nMatch trouvé ligne {idx+1}:")
                print(f"  TVA: {tva}, SIREN: {siren}")
                if match_tva.any():
                    print(f"  Correspondance TVA avec: {df_out.loc[match_tva, 'login_upper'].iloc[0]}")
                if match_siren.any():
                    print(f"  Correspondance SIREN avec: {df_out.loc[match_siren, 'SIREN_str'].iloc[0]}")
    ...
    df_out['canal'] = df_out['role'].apply(
        lambda r: role_to_canal.get(str(r).strip(), "AUTRE") if pd.notna(r) else "AUTRE"
    )

    df_out.to_excel(chemin_out, index=False, engine='openpyxl')
    print(f"\n✅ Correspondances ajoutées dans : {chemin_out}")
    print(f"🔎 {found_count} correspondance(s) trouvée(s) sur {total_lignes_filiale} lignes analysées dans le fichier provenant de la filiale.")

    # ----- Résumé par canal -----
    if found_count > 0:
        summary_canal = df_out[df_out['Correspondance'] == 'found'].groupby('canal').size()
        print("\n📊 Nombre de fournisseurs trouvés par canal :")
        print(summary_canal)
