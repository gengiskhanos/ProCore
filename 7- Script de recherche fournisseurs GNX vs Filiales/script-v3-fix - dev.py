...
def comparer_avec_export(fichier_out='OUT/GENERIX', fichier_export='IN/EXPORT-FILIALE-A-COMPARER'):
    ...
    # Ajout 1 : créer une colonne ident_upper pour comparer la TVA
    df_out['ident_upper'] = df_out['identification'].astype(str).str.strip().str.upper()
    ...
    for idx, (tva, siren) in enumerate(zip(tva_values_clean, siren_values_clean)):
        ...
        # 🔥 Ajout 2 : match TVA sur identification et NON sur login
        match_tva = df_out['ident_upper'] == tva if not pd.isna(tva) else False

        # Match SIREN reste inchangé
    ...
    # 🔥 Ajout 3 : supprimer la colonne ident_upper après usage
    df_out.drop(columns=['login_upper', 'SIREN_str', 'ident_upper'], inplace=True)
